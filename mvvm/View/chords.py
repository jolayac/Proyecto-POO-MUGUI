...
class ChordDetectorView:
    def __init__(self, view_model: ChordDetectorViewModel) -> None:
        self._view_model = view_model

        self.root = tk.Tk()
        self.root.title("Detector de acordes por micrófono")

        self.title_label = tk.Label(self.root, text="Detector de acordes", font=("Helvetica", 18, "bold"))
        self.title_label.pack(pady=10)

        self.chord_label = tk.Label(self.root, text="Pulsa 'Analizar' y toca un acorde", font=("Helvetica", 16))
        self.chord_label.pack(pady=10)

        self.detail_label = tk.Label(self.root, text="", font=("Helvetica", 10), justify=tk.LEFT)
        ...
        self.root.after(200, self._check_result_queue)
    ...
    def _draw_frequency_diagram(
        ...
        for i, f in enumerate(freqs):
            ...
            self.canvas.create_rectangle(x - 5, y0_bar, x + 5, y1, fill="skyblue")

            label = labels[i] if i < len(labels) else ""
            text = f"{label}\n{f:.0f} Hz"
            text_y = max(y0_bar - 2, y1_axis + 2)
            self.canvas.create_text(x, text_y, text=text, font=("Helvetica", 8), anchor="s")
