...
from View.afinador import AudioProcessor
...
def ir_main_menu():
    ...
    menu_biblioteca.add_separator()
    menu_biblioteca.add_command(label="Metrónomo", command=ir_metronomo)
    menu_biblioteca.add_command(label="Afinador", command=ir_detector_notas)
...
def ir_detector_notas():
    ...
    def abrir_afinador():
        ...
        threading.Thread(target=lanzar_afinador, daemon=True).start()

    tk.Button(detector_notas_frame, text="EMPEZAR AFINADOR", 
          command=lambda: threading.Thread(target=lanzar_afinador, daemon=True).start(),
          bg="#fc6e20", fg="black", font=("Arial", 16, "bold")).pack(pady=30)

    tk.Button(detector_notas_frame, 
              text="Volver al Menú", 
              command=ir_main_menu, 
              bg="#555555", 
              fg="white",
...
def ir_metronomo():
    from metronomo import lanzar_metronomo
    threading.Thread(target=lanzar_metronomo, daemon=True).start()
